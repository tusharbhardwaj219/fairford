...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    function cardHTML(p) {
        ...
        // plus a discount badge / margin note.
        let pricingHTML;
        if (!IS_AUTHED) {
      ...
      <div class="pl-card-body">
        ...
        <div class="pl-card-actions">
          <button class="pl-btn-cart${(IS_AUTHED && out) ? ' pl-btn-oos' : ''}" ${!IS_AUTHED ? 'data-login="1"' : `data-cart="${p.id}"`} ${(IS_AUTHED && out) ? 'disabled' : ''}>${cartLabel}</button>
